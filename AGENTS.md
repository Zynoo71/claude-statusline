...
There are no automated tests or linting. Testing is manual — pipe a sample payload into `bin/statusline.sh`, or install the statusline and verify it renders correctly in Claude Code.
...
1. Claude Code pipes JSON context to `statusline.sh` via stdin (includes `rate_limits.{five_hour,seven_day}.{used_percentage,resets_at}`, where `resets_at` is epoch seconds)
