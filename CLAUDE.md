...
bunx @zynoo/claude-statusline --cache-ttl 300
```
...
- **`bin/statusline.sh`** — Bash script that Claude Code invokes. Reads JSON context from stdin (model, tokens, session info), fetches rate limits from the Anthropic API (cached with configurable TTL in `/tmp/claude/`), and outputs a colored multi-line status display.
